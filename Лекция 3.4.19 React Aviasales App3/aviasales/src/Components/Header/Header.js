...
import './Header.scss';
import { getCheapestTickets, getFastestTickets, getOptimalTickets } from "../../async-actions/async-actions";
...
function Header(store) {
  ...
  let first = 'header__cheap title'
  let second = 'header__fast title'
  let third = 'header__optimal title'
  ...
  const onCheap = () => {
    ...
    dispatch(actions.Cheap())
  }

  const onFast = () => {
    dispatch(getFastestTickets())
    dispatch(actions.Fast())
  }
...
const mapStateToProps = (tabsInitialState) => {
  return {
    cheap: tabsInitialState.cheap,
    fast: tabsInitialState.fast,
    optimal: tabsInitialState.optimal
  }
}
